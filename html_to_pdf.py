...
with sync_playwright() as p:
    ...
    for i, url in enumerate(urls, 1):
        ...
        try:
            # 1) tenta a forma mais "completa", mas com timeout menor
            try:
                page.goto(url, wait_until="networkidle", timeout=45000)
            except PWTimeoutError:
                # 2) fallback: carrega DOM e segue
                print("  ⚠ networkidle demorou; usando domcontentloaded + espera extra...")
                page.goto(url, wait_until="domcontentloaded", timeout=120000)

            # espera extra para SPAs renderizarem conteúdo
            page.wait_for_timeout(4000)

            name = filename_from_url(url)
            output_file = f"pdfs/{i:03d}_{name}_{today}.pdf"
            ...
            print(f"  ❌ Falhou nesta URL, mas vou continuar. Erro: {e}")
